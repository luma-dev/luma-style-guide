const processor_options = (exports.processor_options = {});

// find Linter instance
const linter_paths = Object.keys(require.cache).filter(
  (path) => path.endsWith('/eslint/lib/linter/linter.js') || path.endsWith('\\eslint\\lib\\linter\\linter.js'),
);
...
Linter.prototype.verify = function (code, config, options) {
  ...
  const settings =
    (config &&
      (typeof config.extractConfig === 'function' ? config.extractConfig(options.filename) : config).settings) ||
    {};
