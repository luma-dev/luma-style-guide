...
class GeneratedFragmentMapper {
  ...
  get_position_relative_to_fragment(position_relative_to_file) {
    const fragment_offset = this.offset_in_fragment(
      offset_at(position_relative_to_file, this.generated_code),
    );
...
class OriginalFragmentMapper {
  ...
  get_position_relative_to_file(position_relative_to_fragment) {
    const parent_offset = this.offset_in_parent(
      offset_at(position_relative_to_fragment, this.diff.original_content),
    );
...
class SourceMapper {
  ...
  get_original_position(generated_position) {
    ...
    let { line } = generated_position;
    const { column } = generated_position;
    ...
    const column_match = line_match.find(
      (col, idx) =>
...
class DocumentMapper {
  constructor(original_code, generated_code, diffs) {
    ...
    this.mappers = diffs.map((diff) => ({
      ...
      diff: diff.diff,
      generated_fragment_mapper: new GeneratedFragmentMapper(
        generated_code,
        diff,
      ),
  ...
  get_original_position(generated_position) {
    generated_position = {
      line: generated_position.line - 1,
  ...
  map(mapper, generated_position) {
    ...
    const position_in_transpiled_fragment = mapper.generated_fragment_mapper
      .get_position_relative_to_fragment(generated_position);
    // Map the position, using the sourcemap, to the original position in the source fragment
    const position_in_original_fragment = mapper.source_mapper
      .get_original_position(position_in_transpiled_fragment);
    // Map the position to be in the original fragment's parent
    const original_position = mapper.original_fragment_mapper
      .get_position_relative_to_file(position_in_original_fragment);
    return this.to_ESLint_position(original_position);
  }
...
function offset_at(position, text) {
  ...
  const next_line_offset = position.line + 1 < line_offsets.length
    ? line_offsets[position.line + 1]
    : text.length;
